...
export function AtlasDashboard({ initialUser }: { initialUser: User | null }) {
  ...
  async function load() {
    ...
    setBanks(b.foodBanks);
    setItems(i.items);
  }
  useEffect(() => {
    if (user) void load().catch((e) => setNotice(e.message));
  }, [user]);
  ...
  const total = items.reduce((n, i) => n + Number(i.quantity), 0),
    expiring = items.filter(
      (i) =>
        i.expiration_date &&
        new Date(i.expiration_date).getTime() >= Date.now() &&
  ...
  async function importCsv(file: File) {
    ...
    try {
      ...
      form.set("file", file);
      const x = await json("/api/inventory/import-csv", {
        method: "POST",
        body: form,
      });
  ...
  async function edit(item: Item, field: keyof Item, value: string) {
    ...
    try {
      const x = await json(`/api/inventory/items/${item.id}`, {
        ...
        body: JSON.stringify({
          field,
          value: field === "quantity" ? Number(value) : value || null,
    ...
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand">
          ...
          <div>
            <strong>ReliefLink</strong>
            <small>ATLAS operations</small>
          </div>
        </div>
        <div className="site-chip">
          <small>Your food bank</small>
          <strong>{context?.siteName || "Loading…"}</strong>
          <span>
            <i /> {context?.agentName || "Inventory agent"}
          </span>
        </div>
        <nav>
          {(
      ...
      <main>
        <header className="topbar">
          <div>
            <p className="eyebrow">
              {context?.organizationName || "ReliefLink network"}
            </p>
            <h1>
              {
                {
        ...
        {view === "overview" && (
          <>
            <section className="welcome">
              <div>
                <p className="eyebrow">Site-scoped workspace</p>
                <h2>Good to see you, {user.displayName.split(" ")[0]}.</h2>
                <p>
                  Manage {context?.siteName}&apos;s inventory while seeing
                  registered partners across the ReliefLink network.
                </p>
              </div>
              <button
                className="button primary"
                onClick={() => setView("adjustment")}
              >
                Adjust inventory
              </button>
            </section>
            <section className="metric-row">
              <Metric label="Inventory rows" value={items.length} />
              <Metric label="Total units" value={total.toLocaleString()} />
                  ...
                  {banks.slice(0, 5).map((b) => (
                    <div key={b.id}>
                      ...
                      <div>
                        ...
                        <small>
                          {b.address}, {b.county} County
                        </small>
                      </div>
                      <b>{Number(b.inventory_units).toLocaleString()} units</b>
                    </div>
                  ))}
                </div>
                <button className="text-button" onClick={() => setView("map")}>
                  Open network map →
                </button>
              </section>
              <section className="panel">
                <Head
                  kicker="Human-governed agent"
                ...
                <div className="authority">
                  ...
                  <span>
                    No autonomous orders, transfers, reservations, or disposal.
                  </span>
                </div>
                <button
        ...
        {view === "adjustment" && (
          <section>
            ...
            />
            <section className="panel csv-card">
              <div>
        ...
        {view === "inventory" && (
          <section className="panel sheet-panel">
            <div className="sheet-head">
              <Head
                kicker="Live shared ledger"
                title={`${items.length} inventory records`}
              />
              <button
                ...
                <tbody>
                  {items.length ? (
                    items.map((item) => (
                      <tr key={item.id}>
                        <td>
                          <span className={`source-pill ${item.intake_method}`}>
                            {item.intake_method}
                          </span>
                        </td>
                        {columns.map(([field]) => (
                          <td key={field}>
                            <input
                              ...
                              defaultValue={
                                field === "expiration_date" && item[field]
                                  ? String(item[field]).slice(0, 10)
                                  : String(item[field] ?? "")
                              }
                              onKeyDown={(e) =>
                              ...
                              onBlur={(e) =>
                                void edit(item, field, e.target.value)
                              }
                            />
                          </td>
                        ))}
        ...
        {view === "agent" && (
          <section>
            ...
            {agent && (
              <>
                <section className="metric-row">
                  <Metric label="Items monitored" value={agent.summary.items} />
                  <Metric
                    label="Total units"
                    value={agent.summary.totalUnits}
                  />
                  <Metric
                    label="Expiring soon"
                    value={agent.summary.expiring}
                  />
                  <Metric
                    label="Missing locations"
                    value={agent.summary.missingLocations}
                  />
                </section>
                <div className="recommendations">
                  {agent.recommendations.length ? (
                    agent.recommendations.map((r, i) => (
                      <article className="panel" key={r.type}>
...
function Head({ kicker, title }: { kicker: string; title: string }) {
  return (
    <>
      {kicker === "Live shared ledger" && <WarehouseSearch />}
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const body = await json(
        mode === "login" ? "/api/auth/login" : "/api/auth/register-food-bank",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(
    ...
    <main className="auth-shell">
      <section className="auth-story">
        <div className="brand large">
          ...
          <div>
            <strong>ReliefLink</strong>
            <small>Shared food bank operations</small>
          </div>
        </div>
        <div>
      ...
      <section className="auth-card">
        ...
        <form onSubmit={submit}>
          <h2>{mode === "login" ? "Welcome back" : "Join ReliefLink"}</h2>
          <p>
            {mode === "login"
              ? "Access your food bank workspace."
              : "Creates your site, administrator account, map listing, and inventory agent."}
          </p>
          {mode === "register" && (
            <>
              <label>
                Food bank name
                <input name="foodBankName" required />
              </label>
              <label>
                Street address
